...
public class CFontRenderer extends CFont {
   ...
   public float drawStringWithShadow(String text, double x, double y, int color, float scale) {
      float shadowWidth = this.drawString(text, x + 1.0D, y + 1.0D, color, true, scale);
   ...
   public float drawString(String text, float x, float y, int color, float scale) {
      return this.drawString(text, (double)x, (double)y, color, false, scale);
   }
   ...
   public float drawCenteredStringWithShadow(String text, float x, float y, int color, float scale) {
      this.drawString(text, (double)(x - (float)(this.getStringWidth(text) / 2)) + 1.0D, (double)y + 1.0D, color, true, scale);
      return this.drawString(text, x - (float)(this.getStringWidth(text) / 2), y, color, scale);
   }
      ...
      if (text == null) {
         ...
         for(int i = 0; i < size; ++i) {
            ...
            if (character == 167 && i < size) {
               ...
               x += (double)((float)(currentData[character].width - 8 + this.charOffset) * scale);
            }
         }
