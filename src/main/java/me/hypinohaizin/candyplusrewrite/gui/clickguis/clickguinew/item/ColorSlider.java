...
public class ColorSlider extends Component {
   ...
   public float doRender(int mouseX, int mouseY, float x, float y) {
      if (!this.setting.visible()) {
         ...
         RenderUtil.drawRect(x + this.width - 20.0F, rcy2, 10.0F, 10.0F, ColorUtil.toRGBA(new Color(((Color)this.setting.getValue()).getRed(), ((Color)this.setting.getValue()).getGreen(), ((Color)this.setting.getValue()).getBlue(), ((Color)this.setting.getValue()).getAlpha())));
   ...
   public void onMouseClickMove(int mouseX, int mouseY, int clickedMouseButton) {
      if (this.isOpening) {
         ...
         float colorFieldY = this.y + this.height;
         float colorFieldHeightY = colorFieldY + 70.0F;
         if (this.x + 8.0F < (float)mouseX && this.x + this.width - 12.0F > (float)mouseX && clickedMouseButton == 0 && this.redChanging) {
            this.rLx = ((float)mouseX - (this.x + 10.0F)) / (this.width - 20.0F);
            if (this.rLx > 1.0F) {
               this.rLx = 1.0F;
            }
         ...
         if (this.x + 8.0F < (float)mouseX && this.x + this.width - 12.0F > (float)mouseX && clickedMouseButton == 0 && this.greenChanging) {
            this.gLx = ((float)mouseX - (this.x + 10.0F)) / (this.width - 20.0F);
            if (this.gLx > 1.0F) {
               this.gLx = 1.0F;
            }
         ...
         if (this.x + 8.0F < (float)mouseX && this.x + this.width - 12.0F > (float)mouseX && clickedMouseButton == 0 && this.blueChanging) {
            this.bLx = ((float)mouseX - (this.x + 10.0F)) / (this.width - 20.0F);
            if (this.bLx > 1.0F) {
               this.bLx = 1.0F;
            }
         ...
         if (this.x + 8.0F < (float)mouseX && this.x + this.width - 12.0F > (float)mouseX && clickedMouseButton == 0 && this.alphaChanging) {
            this.aLx = ((float)mouseX - (this.x + 10.0F)) / (this.width - 20.0F);
            if (this.aLx > 1.0F) {
               this.aLx = 1.0F;
            }
