...
public class PlayerUtil implements Util {
   ...
   public static EntityPlayer getNearestPlayer(double range) {
      return (EntityPlayer)mc.world.playerEntities.stream().filter((p) -> {
         return (double)mc.player.getDistance(p) <= range;
      }).filter((p) -> {
      ...
      for(int j = 0; j < (int)range; ++j) {
         for(int k = 2; k > 0; --k) {
            ...
            while(var9.hasNext()) {
               ...
               double xArray = positionEyes.x + rotationEyes.x * (double)j + rotationEyes.x / (double)k;
               double yArray = positionEyes.y + rotationEyes.y * (double)j + rotationEyes.y / (double)k;
               double zArray = positionEyes.z + rotationEyes.z * (double)j + rotationEyes.z / (double)k;
   ...
   public static double getDistance(Entity entity) {
      return entity != null && mc != null && mc.player != null ? (double)mc.player.getDistance(entity) : Double.MAX_VALUE;
   }

   public static double getDistance(BlockPos pos) {
      return pos != null && mc != null && mc.player != null ? mc.player.getDistance((double)pos.getX(), (double)pos.getY(), (double)pos.getZ()) : Double.MAX_VALUE;
   }

   public static double getDistanceI(BlockPos pos) {
      return pos != null && mc != null && mc.player != null ? getPlayerPos(mc.player).getDistance(pos.getX(), pos.getY(), pos.getZ()) : Double.MAX_VALUE;
   }

   public static BlockPos min(List<BlockPos> posList) {
      return (BlockPos)posList.stream().min(Comparator.comparing((b) -> {
            ...
            for(int y = MathHelper.floor(playerBoundingBox.minY); y < MathHelper.floor(playerBoundingBox.maxY) + 1; ++y) {
               for(int z = MathHelper.floor(playerBoundingBox.minZ); z < MathHelper.floor(playerBoundingBox.maxZ) + 1; ++z) {
                  ...
                  if (!(block instanceof BlockAir)) {
                     AxisAlignedBB boundingBox = block.getCollisionBoundingBox(mc.world.getBlockState(new BlockPos(x, y, z)), mc.world, new BlockPos(x, y, z)).offset((double)x, (double)y, (double)z);
   ...
   public static void setPosition(BlockPos pos) {
      mc.player.setPosition((double)pos.getX() + 0.5D, (double)pos.getY(), (double)pos.getZ() + 0.5D);
   }
