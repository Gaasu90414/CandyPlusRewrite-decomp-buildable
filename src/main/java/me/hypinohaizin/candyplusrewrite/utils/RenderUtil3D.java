...
public class RenderUtil3D implements Util {
   ...
   public static void drawBoundingBox(BlockPos bp, double height, float width, Color color) {
      drawBoundingBox(getBoundingBox(bp, 1.0D, height, 1.0D), (double)width, color, color.getAlpha());
   }
   ...
   public static void drawBoxWithDirection(AxisAlignedBB bb, Color color, float rotation, float width, int mode) {
      ...
      RenderUtil3D.Points square = new RenderUtil3D.Points(bb.minY, bb.maxY, xCenter, zCenter, rotation);
      if (mode == 0) {
   ...
   public static void drawDirection(RenderUtil3D.Points square, Color color, float width) {
      int i;
      for(i = 0; i < 4; ++i) {
