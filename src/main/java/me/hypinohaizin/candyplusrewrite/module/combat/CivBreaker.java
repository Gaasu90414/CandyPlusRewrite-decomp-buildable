...
public class CivBreaker extends Module {
   ...
   public Setting<CivBreaker.Target> targetType;
   ...
   private BlockPos lockedBase = null;
   ...
   public CivBreaker() {
      ...
      this.targetType = this.register(new Setting("Target", CivBreaker.Target.Nearest));
   ...
   public void doCV() {
      if (!this.nullCheck()) {
         try {
            ...
            if (this.lockedBase == null) {
               if (!this.findSpace(target)) {
                  if ((Boolean)this.toggle.getValue()) {
               ...
               this.lockedBase = this.base;
               this.lockTicks = 20;
            } else {
               this.base = this.lockedBase;
               if (this.lockTicks > 0) {
                  --this.lockTicks;
               }
            }
            ...
            if (!this.builtTrap && BlockUtil.getBlock(this.base) == Blocks.AIR && this.blockTimer.passedX((double)(Float)this.preDelay.getValue())) {
               this.setItem(this.obbySlot);
               this.placeBlock(this.base, false);
               this.blockTimer.reset();
               this.builtTrap = true;
               this.tryPlaceCrystalImmediate();
            }
            ...
            if (this.builtTrap && !this.placedCrystal && this.crystalTimer.passedX((double)(Float)this.crystalDelay.getValue())) {
               if (CrystalUtil.canPlaceCrystal(this.base)) {
                  ...
                  if ((Boolean)this.packetCrystal.getValue()) {
                     mc.player.connection.sendPacket(new CPacketPlayerTryUseItemOnBlock(this.base, EnumFacing.UP, hand, 0.5F, 1.0F, 0.5F));
                  } else {
                     mc.playerController.processRightClickBlock(mc.player, mc.world, this.base, EnumFacing.UP, new Vec3d(0.5D, 1.0D, 0.5D), hand);
                  }
            ...
            if (this.placedCrystal && !this.brokeBlock && this.breakTimer.passedX((double)(Float)this.breakDelay.getValue())) {
               ...
               if (!breaking) {
                  if (!(Boolean)this.instantBreak.getValue()) {
                     ...
                     mc.player.connection.sendPacket(new CPacketPlayerDigging(Action.START_DESTROY_BLOCK, this.base, EnumFacing.DOWN));
                     mc.player.connection.sendPacket(new CPacketPlayerDigging(Action.STOP_DESTROY_BLOCK, this.base, EnumFacing.DOWN));
                  } else {
                     if (!(Boolean)this.noSwingBlock.getValue()) {
                        mc.player.swingArm(EnumHand.MAIN_HAND);
                     }
            ...
            if (this.brokeBlock && !this.attackedCrystal) {
               ...
               if (this.attackTimer.passedX((double)(Float)this.attackDelay.getValue())) {
                  BlockPos plannedCrystalPos = this.base.add(0, 1, 0);
   ...
   private void tryPlaceCrystalImmediate() {
   ...
   public void onRender3D() {
      try {
         if (this.isNull(target) || this.isNull(this.base)) {
            return;
         }

         RenderUtil3D.drawBox(this.base, 1.0D, (Color)this.blockColor.getValue(), 63);
         if ((Boolean)this.outline.getValue()) {
            RenderUtil3D.drawBoundingBox(this.base, 1.0D, (Float)this.width.getValue(), (Color)this.blockColor.getValue());
         }
   ...
   public boolean findSpace(EntityPlayer player) {
      ...
      for(int var7 = 0; var7 < var6; ++var7) {
         ...
         if ((BlockUtil.getBlock(basePos) == Blocks.OBSIDIAN || BlockUtil.getBlock(basePos) == Blocks.BEDROCK) && (BlockUtil.canPlaceBlockFuture(basePos.up()) || BlockUtil.getBlock(basePos.up()) == Blocks.OBSIDIAN) && CrystalUtil.canPlaceCrystal(basePos.up())) {
            posess.add(basePos.up());
         }
      }
   ...
   public EntityPlayer findTarget() {
      ...
      if (candidates.isEmpty()) {
         ...
         switch((CivBreaker.Target)this.targetType.getValue()) {
         case Nearest:
